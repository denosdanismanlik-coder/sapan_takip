...
class LoginPage extends StatelessWidget {
  ...
  Future<void> kayit(BuildContext context) async {
    try {
...
class _SapanListesiState extends State<SapanListesi> {
  ...
  Future<void> excelIndir() async {
    ...
    // Bu sürüm mobil + web ortak derlensin diye dart:html kullanmaz.
    // Excel indirme kısmını sonraki adımda platforma özel dosya ile tekrar açacağız.
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Excel hazırlandı. İndirme özelliğini fotoğraf sorunu çözüldükten sonra tekrar aktif edeceğiz.")),
    );
  }
